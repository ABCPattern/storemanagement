...
exports.getAdmin = async (req, res) => {
    ...
    const data = await Admin.find().populate('categoryId').select('-password').limit(per_page).skip(skipitems)//output is promise hence use of then and catch method
    if (!data) {
        res.status(500)
        res.json({
            success: false,
...
exports.adminInfo = async (req, res) => {
    ...
    const info = await Admin.findById(id).select('-password')
    if (!info) {
        res.status(500)
        res.json({
            success: false,
...
exports.updateadmin = async (req, res) => {
    try {
        ...
        if (!sadmin) {
            res.status(404);
            res.send("The superadmin does not exist");
            return;
        }
        if (!adminid) {
            res.status(404);
